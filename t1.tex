...
The useful operation of this feature fundamentally relies on process polymorphism.
...
a multi-receive can produce a list of sender--value pairs, one for each message received.

%Even the simplest version of a multi-receive that just removes ordering constraints as compared to a sequence of individual receives provides value.
%With ordered receives, senders later in the list must wait on senders earlier in the list before proceeding
%
%While the simplest version of multi-receive simply removes ordering constraints from a sequence of individual receives,
%its real power becomes evident when we only require a subset of the messages to arrive before the recipient continues.
%This is a fundamental primitive needed to build many systems, such as distributed consensus protocols like Paxos~\citep{Lamport98}.
